...
const Home = () => {
    ...
    const handleChangeInputs = async (e) => {
        ...
        if ((name === 'cpf' && value.length >= 3) || (name === 'name' && value.length >= 3)) {
            const people = await findPeople({ ...formData, [name]: value }, setError, setMsg);
            setResults(people);
        }
    }
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        await findPeople(formData, setError, setMsg);
    }
    ...
    return (
        <div className="flex flex-col col-span-1 min-h-screen pt-5 items-center bg-white">
            <div className="relative flex flex-col rounded-xl bg-transparent bg-clip-border text-gray-700 shadow-none">
                ...
                <form className="mt-4 mb-2 w-80 max-w-screen-lg sm:w-96" onSubmit={handleSubmit} method="POST">
                    <InputCpf
                        name="cpf"
